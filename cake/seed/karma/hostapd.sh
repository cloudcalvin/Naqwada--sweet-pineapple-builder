...
hostapd_set_bss_options() {
	...
	case "$auth_type" in
		...
		eap)
			...
			append bss_conf "eapol_key_index_workaround=0" "$N"
			append bss_conf "eap_server=1" "$N"
			append bss_conf "eap_fast_a_id=101112131415161718191a1b1c1d1e1f" "$N"
			append bss_conf "eap_fast_a_id_info=hostapd-pineape" "$N"
			append bss_conf "eap_fast_prov=3" "$N"
			append bss_conf "ieee8021x=1" "$N"
			append wpa_key_mgmt "WPA-EAP"
			append bss_conf "pac_key_lifetime=604800" "$N"
			append bss_conf "pac_key_refresh_time=86400" "$N"
			append bss_conf "pac_opaque_encr_key=000102030405060708090a0b0c0d0e0f" "$N"
			append bss_conf "wpa_pairwise=CCMP" "$N"
			append bss_conf "rsn_pairwise=CCMP" "$N"
			append bss_conf "eap_user_file=/etc/pineape/hostapd-pineape.eap_user" "$N"
			append bss_conf "ca_cert=/etc/pineape/certs/ca.pem" "$N"
			append bss_conf "dh_file=/etc/pineape/certs/dh" "$N"
			append bss_conf "server_cert=/etc/pineape/certs/server.pem" "$N"
			append bss_conf "private_key=/etc/pineape/certs/server.key" "$N"
			append bss_conf "private_key_passwd=pineapplesareyummy" "$N"
...
hostapd_set_log_options() {
	...
	append "$var" "logger_syslog=-1" "$N"
	# append "$var" "logger_syslog_level=$log_level" "$N"
	append "$var" "logger_syslog_level=0" "$N"
