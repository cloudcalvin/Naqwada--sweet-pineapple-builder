...
drv_mac80211_setup() {
	...
	[ -n "$hostapd_ctrl" ] && {
		...
		sleep 2
        hostapd_cli -i wlan0 karma_mac_$(uci get pineap.@config[0].mac_filter)
        hostapd_cli -i wlan0 karma_$(uci get pineap.@config[0].ssid_filter)
        hostapd_cli -i wlan0 karma_logging_enable
        hostapd_cli -i wlan0-2 karma_logging_enable
        [[ "$(uci get pineap.@config[0].karma)" = "on" ]] && hostapd_cli -i wlan0 karma_enable

        hostapd_cli -i wlan0-2 pineape_auth_passthrough_$(uci get pineap.@config[0].pineape_passthrough)
	}
